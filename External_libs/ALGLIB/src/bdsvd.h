...
namespace bdsvd
{
    ...
    *************************************************************************/
        ...
        int ncvt)
    {
        ...
        if( n==1 )
        {
            if( d(1)<0 )
            {
                ...
                if( ncvt>0 )
                {
                    amp::vmul(vt.getrow(vstart, vstart, vstart+ncvt-1), -1);
                }
            }
            return result;
        }
        ...
        //
        tolmul = amp::maximum<Precision>(amp::ampf<Precision>(10), amp::minimum<Precision>(amp::ampf<Precision>(100), amp::pow<Precision>(eps, -amp::ampf<Precision>("0.125"))));
        tol = tolmul*eps;
        ...
        if( tol>=0 )
        {
            ...
            sminoa = sminoa/amp::sqrt<Precision>(amp::ampf<Precision>(n));
        ...
        while( true )
        {
            ...
            if( ll==m-1 )
            {
                ...
                if( ncvt>0 )
                {
                    mm0 = m+(vstart-1);
                    mm1 = m-1+(vstart-1);
                    amp::vmove(vttemp.getvector(vstart, vend), vt.getrow(mm1, vstart, vend), cosr);
                    amp::vadd(vttemp.getvector(vstart, vend), vt.getrow(mm0, vstart, vend), sinr);
                    amp::vmul(vt.getrow(mm0, vstart, vend), cosr);
                    amp::vsub(vt.getrow(mm0, vstart, vend), vt.getrow(mm1, vstart, vend), sinr);
                    amp::vmove(vt.getrow(mm1, vstart, vend), vttemp.getvector(vstart, vend));
                }
                if( nru>0 )
                {
                    mm0 = m+ustart-1;
                    mm1 = m-1+ustart-1;
                    amp::vmove(utemp.getvector(ustart, uend), u.getcolumn(mm1, ustart, uend), cosl);
                    amp::vadd(utemp.getvector(ustart, uend), u.getcolumn(mm0, ustart, uend), sinl);
                    amp::vmul(u.getcolumn(mm0, ustart, uend), cosl);
                    amp::vsub(u.getcolumn(mm0, ustart, uend), u.getcolumn(mm1, ustart, uend), sinl);
                    amp::vmove(u.getcolumn(mm1, ustart, uend), utemp.getvector(ustart, uend));
                }
                if( ncc>0 )
                {
                    mm0 = m+cstart-1;
                    mm1 = m-1+cstart-1;
                    amp::vmove(ctemp.getvector(cstart, cend), c.getrow(mm1, cstart, cend), cosl);
                    amp::vadd(ctemp.getvector(cstart, cend), c.getrow(mm0, cstart, cend), sinl);
                    amp::vmul(c.getrow(mm0, cstart, cend), cosl);
                    amp::vsub(c.getrow(mm0, cstart, cend), c.getrow(mm1, cstart, cend), sinl);
                    amp::vmove(c.getrow(mm1, cstart, cend), ctemp.getvector(cstart, cend));
                }
                m = m-2;
                continue;
            }
            ...
            else
            {
                ...
                if( idir==1 )
                {
                    ...
                    // Save cosines and sines for later singular vector updates
                    //
                    f = (amp::abs<Precision>(d(ll))-shift)*(extsignbdsqr<Precision>(amp::ampf<Precision>(1), d(ll))+shift/d(ll));
                    g = e(ll);
                    for(i=ll; i<=m-1; i++)
                    {
                ...
                else
                {
                    ...
                    // Save cosines and sines for later singular vector updates
                    //
                    f = (amp::abs<Precision>(d(m))-shift)*(extsignbdsqr<Precision>(amp::ampf<Precision>(1), d(m))+shift/d(m));
                    g = e(m-1);
                    for(i=m; i>=ll+1; i--)
                    {
                        rotations::generaterotation<Precision>(f, g, cosr, sinr, r);
        ...
        for(i=1; i<=n; i++)
        {
            if( d(i)<0 )
            {
                ...
                if( ncvt>0 )
                {
                    amp::vmul(vt.getrow(i+vstart-1, vstart, vend), -1);
                }
            }
        }
        ...
        for(i=1; i<=n-1; i++)
        {
            ...
            if( isub!=n+1-i )
            {
                ...
                {
                    j = n+1-i;
                    amp::vmove(vttemp.getvector(vstart, vend), vt.getrow(isub+vstart-1, vstart, vend));
                    amp::vmove(vt.getrow(isub+vstart-1, vstart, vend), vt.getrow(j+vstart-1, vstart, vend));
                    amp::vmove(vt.getrow(j+vstart-1, vstart, vend), vttemp.getvector(vstart, vend));
                }
                if( nru>0 )
                {
                    j = n+1-i;
                    amp::vmove(utemp.getvector(ustart, uend), u.getcolumn(isub+ustart-1, ustart, uend));
                    amp::vmove(u.getcolumn(isub+ustart-1, ustart, uend), u.getcolumn(j+ustart-1, ustart, uend));
                    amp::vmove(u.getcolumn(j+ustart-1, ustart, uend), utemp.getvector(ustart, uend));
                }
                if( ncc>0 )
                {
                    j = n+1-i;
                    amp::vmove(ctemp.getvector(cstart, cend), c.getrow(isub+cstart-1, cstart, cend));
                    amp::vmove(c.getrow(isub+cstart-1, cstart, cend), c.getrow(j+cstart-1, cstart, cend));
                    amp::vmove(c.getrow(j+cstart-1, cstart, cend), ctemp.getvector(cstart, cend));
                }
            }
        }
        return result;
        ...
        amp::ampf<Precision>& csl)
    {
        ...
        else
        {
            ...
            if( gasmal )
            {
                ...
                if( mm==0 )
                {
                    ...
                    if( l==0 )
                    {
                        t = extsignbdsqr<Precision>(amp::ampf<Precision>(2), ft)*extsignbdsqr<Precision>(amp::ampf<Precision>(1), gt);
                    }
                    else
                    {
        ...
        if( pmax==1 )
        {
            tsign = extsignbdsqr<Precision>(amp::ampf<Precision>(1), csr)*extsignbdsqr<Precision>(amp::ampf<Precision>(1), csl)*extsignbdsqr<Precision>(amp::ampf<Precision>(1), f);
        }
        if( pmax==2 )
        {
            tsign = extsignbdsqr<Precision>(amp::ampf<Precision>(1), snr)*extsignbdsqr<Precision>(amp::ampf<Precision>(1), csl)*extsignbdsqr<Precision>(amp::ampf<Precision>(1), g);
        }
        if( pmax==3 )
        {
            tsign = extsignbdsqr<Precision>(amp::ampf<Precision>(1), snr)*extsignbdsqr<Precision>(amp::ampf<Precision>(1), snl)*extsignbdsqr<Precision>(amp::ampf<Precision>(1), h);
        }
        ssmax = extsignbdsqr<Precision>(ssmax, tsign);
        ssmin = extsignbdsqr<Precision>(ssmin, tsign*extsignbdsqr<Precision>(amp::ampf<Precision>(1), f)*extsignbdsqr<Precision>(amp::ampf<Precision>(1), h));
    }
} // namespace
