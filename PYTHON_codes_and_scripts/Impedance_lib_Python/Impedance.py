...
def Cu20K_layer(thickness=50e-6,RRR=70,B=7000/(2803.9*0.299792458)):
    ...
    rhoDC=2.4e-10*(1.0048+0.0038*RRR*B);
...
def G_element(m,x,Gint):
    ...
    if Gint==0:
    	return np.tanh((2*m+1)*np.pi*x/2)/(np.cosh((2*m+1)*np.pi*x/2)**2)/(2*m+1);
    
    elif Gint==1:
    	return x**3*(2*m+1)/(np.tanh((2*m+1)*np.pi*x/2)*np.sinh((2*m+1)*np.pi*x/2)**2);

    elif Gint==2:
    	return x**2*(2*m+1)*np.tanh((2*m+1)*np.pi*x/2)/(np.cosh((2*m+1)*np.pi*x/2)**2);

    elif Gint==3:
    	return x**2*(2*m)*np.tanh(m*np.pi*x)/(np.cosh(m*np.pi*x)**2);
...
def imp_model_from_IW2D(iw_input,wake_calc=False,path=IW2D_path,
    ...
    imp_model=[];wake_model=[];
	
    # list of different components
    listname=['long','xdip','ydip','xquad','yquad','ycst'];
    listplane=['z','x','y','x','y','y'];
    lista=[0,1,0,0,0,0];listb=[0,0,1,0,0,0];
    listc=[0,0,0,1,0,0];listd=[0,0,0,0,1,0];
    ...
    if (len(iw_input.b)>=2): ncomp=6; # include cst y component (no top-bottom symmetry)
    else: ncomp=5; # top-bottom symmetry -> Zycst=Wycst=0
	...
	filejob=open('batch'+iw_input.comment+'.job','w');
	here=os.getcwd();#print here;
	print >> filejob, "cp "+here+"/"+filename+" .";
	print >> filejob, "cp "+here+"/"+wakestr+iw_input.geometry+"chamber.x .";
	print >> filejob, "./"+wakestr+iw_input.geometry+"chamber.x < "+filename;
	...
	if wake_calc: suffix=suffix1.replace('.dat','_precise.dat');
	else: suffix=suffix1;
	...
	if wake_calc:
		...
		W=np.zeros((len(s[:,0]),2));W[:,0]=s[:,1];
		wake_model.append(impedance_wake(a=lista[icomp],b=listb[icomp],c=listc[icomp],
			d=listd[icomp],plane=listplane[icomp],var=s[:,0],func=W));
...
def imp_model_elliptic(iw_input,w,orientation='V',wake_calc=False,
    ...
    imp_model,wake_model=imp_model_from_IW2D(iw_input,wake_calc=wake_calc,flagrm=flagrm,lxplusbatch=lxplusbatch,queue=queue,dire=dire)
...
def power_loss(imp_mod,sigz,gamma,Nb,M,circum,powerspectrum='gaussian',particle='proton'):
	...
	while (p[-1]<2.*pmax)or(np.abs(s/Ploss)>1.e-7):
	    ...
	    if (p[-1]*M*f0>freq[-1]):
        	disp('power_loss: warning: interpolating above the upper limit of Zlong in frequency')
	...
	for inx,nx in enumerate(nxscan): # coupled-bunch modes
		...
		for im,m in enumerate(range(-mmax,mmax+1)):
		    ...
		    Zeff[iQp,inx,iomegas,im]=sum2/sum1;
        	    freqshift =( 1j*e*beta*Ibscan/(2.*(np.abs(m)+1.)*m0*gamma*Q*omega0*Lb) ) * sum2/sum1;
